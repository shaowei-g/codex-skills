...
`booster` mode does not change the delegated prompt shape. It only allows the orchestrator to issue a new delegated prompt for a later phase after the current delegated step is accepted.
