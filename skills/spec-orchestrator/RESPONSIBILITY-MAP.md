...
Everything else should live in one canonical reference or the owning specialist skill. Specialists should load only a thin execution contract plus the shared response schema, not orchestration mechanics.
...
| `references/specialist-execution-contract.md` | Thin common contract for specialists | one bounded pass, no routing authority, advisory-only recommendations, schema-only return, stop rule | orchestration lifecycle, reinjection policy, transport fallback, validator repair flow |
...
- If a rule applies to every specialist run but does not require orchestration mechanics, keep it in `references/specialist-execution-contract.md`.
- If a rule applies to every delegated run at the orchestrator layer, prefer an orchestrator-owned reference over repeating it in each specialist skill.
...
- If a rule is about specialist execution boundaries such as one bounded pass, no routing authority, or stop-after-return, keep it only in `references/specialist-execution-contract.md`.
- If a rule is about delegation reinjection, validator repair, or orchestrator-owned authority semantics, keep it only in `references/subagent-reinjection-contract.md`.
